...
(function () {
    ...
    math.variance = function (arr) {
        var mean = _(arr).mean();
        return _(arr).chain().map(function (x) {
...
(function (global, _) {
    ...
    Miso.Events = {
        ...
        unsubscribe : function (name, identifier) {
            ...
            if (_.isUndefined(this._events[name])) {
                return this;
            }
...
(function (global, _) {
    ...
    _.extend(Dataset.DataView.prototype, {
        ...
        _column : function (name) {
            if (_.isUndefined(this._columnPositionByName)) {
                return undefined;
            }
        ...
        cacheRows : function (dataset) {
            ...
            if (rowLengths.length > 1) {
                throw new Error("Row lengths need to be the same. Empty values should be set to null." +
                    _.map(dataset._columns, function (c) {
                        return c.data + "|||";
                    }));
            }
            else {
...
(function (global, _) {
    ...
    _.extend(Dataset.DataView.prototype, {
        ...
        sum : Dataset.Product.define(function (columns, options) {
            ...
            return _.sum(_.map(columns, function (c) {
                return c._sum();
            }));
        }),
        ...
        mean : Dataset.Product.define(function (columns, options) {
            ...
            vals = _.map(vals, function (v) {
                return Dataset.types[type].numeric(v);
            });
            return _.mean(vals);
        })
                ...
                _.each(columns, function (colName) {
                    ...
                    _.each(column.data, function (bin, binPos) {
                        if (_.isArray(bin)) {
                            ...
                            oidcol.data[binPos].push(_.map(bin, function (row) {
                                return row[self.parent.idAttribute];
                            }));
                            oidcol.data[binPos] = _.flatten(oidcol.data[binPos]);

                            // compute the final value.
                            column.data[binPos] = this.method(_.map(bin, function (row) {
                                return row[colName];
                            }));
                            this.length++;
                        }
